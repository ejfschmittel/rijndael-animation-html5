...
var JES = {
		...
		encrypt: function (opts, output_type, decrypt) {
			...
			keys = this.keySchedule(this.toKey(opts.key), 10);  // expand keysd?
			...
			for (i = 0, j = blocks.length; i < j; i += 1) {
				...
				if (!decrypt) {
					...
					this.internalState[`block-${i}-input-state`] = state
					...
					for (k = 1, l = 9; k <= l; k += 1) {
                        ...
                        this.internalState[`block-${i}-mix-columns-${k}`] = state
                        state = this.addRoundKey(state, keys[k])
                        this.internalState[`block-${i}-add-round-key-${k}`] = state
                        this.internalState[`round-key-${k}`] = keys[k]
						//state = this.addRoundKey(this.mixColumns(this.shiftRows(this.subBytes(state))), keys[k]);
					}

                    state = this.subBytes(state)
                    this.internalState[`block-${i}-sub-bytes-final`] = state
                    state = this.shiftRows(state)
                    this.internalState[`block-${i}-shift-rows-final`] = state
					state = this.addRoundKey(state, keys[k]);
                    this.internalState[`block-${i}-add-round-key-final`] = state
                    this.internalState[`round-key-10`] = keys[k]
				} else {
